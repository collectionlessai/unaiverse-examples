...
class WAgent(Agent):
    ...
    async def on_tick(self):
        ...
        if self.__hotel_manager is not None and await self.disconnected(agent=self.__hotel_manager):
            log.user("❌ Oops! Lost connection to the hotel manager!")
        ...
        # Checking if we lost connection to the floor manager
            ...
            log.user("❌ Oops! Lost connection to the floor manager!")
        ...
        if ((state is None or state.name != "init") and
            ...
            log.user("❌ Too much time passed without interactions. Time to leave the hotel for a while.")
            await self.disconnect_hotel_manager()  # Disconnecting hotel manager (will clear too)
            await self.disconnect_floor_manager()  # Disconnecting floor manager too (will clear too)
    ...
    async def init(self):
        if not self._init_message_printed:
            ...
            if not self.__is_in_form_filled_user_list(nickname, is_human):
                form_link = Config.form[is_human].replace("<YOUR_NICKNAME>", nickname)
                init_message = Config.init_message_with_form.replace("<FORM_LINK>", form_link)
            else:
    ...
    async def send_msg(self):
        ...
        if len(self.__queued_msgs) >= Config.max_queued_msgs and not self.__flood_warned:
            ...
            log.user(f"⚠️ Messages are arriving too fast. I can send one message "
                     f"every {Config.msg_cooldown:g} seconds; older queued messages may be dropped.")
        elif cooldown_left > 0. and len(self.__queued_msgs) == 1:
            log.user(f"⏳ Anti-flooding cooldown: the message will be sent in {cooldown_left:.1f} seconds.")
        ...
        if len(self.__queued_msgs) > Config.max_queued_msgs:
            ...
            log.user(f"⏳ You are typing too fast: dropped {dropped} queued message(s).")
    ...
    async def __flush_queued_msgs(self) -> None:
        ...
        if Config.max_message_size > 0:
            if len(msg) > Config.max_message_size:
                ...
                log.user(f"✂️ Message too long: sending only the first {Config.max_message_size} characters.")
    ...
    def __is_in_form_filled_user_list(self, nickname: str, is_human: bool) -> bool:
        if (not hasattr(Config, "registered_users_form_sheets") or
            ...
            return True
        if not Config.registered_users_form_sheets:
            return True
        ...
        url = form_sheets[is_human].rstrip("/") + "/gviz/tq?tqx=out:csv"
        try:
            try:
