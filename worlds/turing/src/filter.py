...
_WORDLISTS_ENV = "TURING_WORDLISTS"  # Escape hatch: point it at the folder and skip the search
...
def _wordlists_dir() -> str:
    ...
    hosted from elsewhere: running "uv run .../turing/run_w.py" from the repository root puts the
    ...
    raise FileNotFoundError(f"Could not locate the turing 'wordlists' directory: looked {next_to}under every "
...
def test_rule_based_filter() -> None:
    ...
    Run it with:  python -m worlds.turing.src.filter
    """
    import asyncio
    ...
    # English chat and the English labels shown when personal data is masked.
    for msg in ["Hi Grace, how was your day?", "I bought a book and went for a walk.",
                "I love classical music and live near Scunthorpe."]:
        assert not check(msg).hits, f"false positive on {msg!r}: {check(msg)}"
    assert check("What the fuck?").clean_msg == "What the ***?"
    assert check("this is bull shit").clean_msg == "this is ***"
    assert check("Email guest@example.org").clean_msg == "Email [email]"
    assert check("My card is 4539578763621486").clean_msg == "My card is [card]"
    ...
    assert r.clean_msg == "scrivimi a [email] oppure al [phone]", r.clean_msg
